...
_DOTPROD_BENCH_TOPK = _env_int("GHOST_DOTPROD_BENCH_TOPK", 30)
_DOTPROD_BENCH_EXPORT_PATH = os.getenv("GHOST_DOTPROD_BENCH_EXPORT_PATH")  # optional file path
...
def _format_dotprod_bench_summary(top_k: int) -> str:
    with _DOTPROD_BENCH_LOCK:
        items = list(_DOTPROD_BENCH_STATS.items())
    rows = []
    for name, st in items:
        if st.measured <= 0:
            continue
        avg_ms = (st.total_s / st.measured) * 1e3
        rows.append((avg_ms, name, st.measured, st.min_s * 1e3, st.max_s * 1e3))
    rows.sort(reverse=True, key=lambda x: x[0])
    rows = rows[: max(0, int(top_k))]
    lines = ["[ghost dotprod bench] ===== per-layer summary (avg_ms desc) ====="]
    for avg_ms, name, measured, min_ms, max_ms in rows:
        lines.append(
            f"[ghost dotprod bench] {name} avg_ms={avg_ms:.3f} measured={measured} min_ms={min_ms:.3f} max_ms={max_ms:.3f}"
        )
    return "\n".join(lines)


def print_dotprod_bench_summary(top_k: Optional[int] = None) -> None:
    if not _DOTPROD_BENCH_ENABLED:
        return
    k = _DOTPROD_BENCH_TOPK if top_k is None else int(top_k)
    if k <= 0:
        return
    print(_format_dotprod_bench_summary(k))


def get_dotprod_bench_topk_layer_names(top_k: Optional[int] = None) -> list[str]:
    """
    Returns the top-k slowest layer names (by avg_ms) seen by dotprod bench.
    These names are suitable to be passed to include_layer_names.
    """
    if not _DOTPROD_BENCH_ENABLED:
        return []
    k = _DOTPROD_BENCH_TOPK if top_k is None else int(top_k)
    ...
    for name, st in items:
        if st.measured <= 0:
            continue
        avg_ms = (st.total_s / st.measured) * 1e3
        rows.append((avg_ms, name))
    rows.sort(reverse=True, key=lambda x: x[0])
    return [name for _, name in rows[:k]]


def export_dotprod_bench(
    export_path: str,
    top_k: Optional[int] = None,
) -> None:
    """
    Export bench summary to a JSON file, including a ready-to-use include_layer_names list.
    """
    if not _DOTPROD_BENCH_ENABLED:
        return
    import json
        ...
        rows.append(
            {
                ...
                "avg_ms": float((st.total_s / st.measured) * 1e3),
                "min_ms": float(st.min_s * 1e3),
                "max_ms": float(st.max_s * 1e3),
            }
        )
    rows.sort(key=lambda r: r["avg_ms"], reverse=True)
    top_layers = [r["layer_name"] for r in rows[:k]]
    payload = {
        "top_k": int(k),
        "warmup": int(_DOTPROD_BENCH_WARMUP),
        "sync": bool(_DOTPROD_BENCH_SYNC),
        "layers": rows,
        "include_layer_names_topk": top_layers,
    }
    os.makedirs(os.path.dirname(export_path) or ".", exist_ok=True)
    with open(export_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print(f"[ghost dotprod bench] exported to {export_path}")
...
def add_hooks(
    ...
    include_layer_names: Optional[list[str]] = None,
):
    r"""
    ...
    if include_layer_names:
        include_set = set(include_layer_names)
...
def remove_hooks(model: nn.Module):
    ...
    # Emit a bench summary at teardown so short profiling runs are easy to parse.
    try:
        print_dotprod_bench_summary()
    except Exception:
        pass
    # Optionally export bench stats for automated hotspot selection.
    if _DOTPROD_BENCH_ENABLED and _DOTPROD_BENCH_EXPORT_PATH:
        try:
            export_dotprod_bench(_DOTPROD_BENCH_EXPORT_PATH)
        except Exception as e:
            warnings.warn(f"Failed to export dotprod bench stats: {e}")
