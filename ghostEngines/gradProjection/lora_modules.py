...
class LoraLinear(nn.Module):
    def __init__(self, rank: int, linear: nn.Linear, shared_module: nn.Linear = None):
        ...
        rank1, rank2 = self.rank, self.rank

        self.logix_lora_A = nn.Linear(in_features, rank1, bias=False)
        self.logix_lora_B = shared_module or nn.Linear(rank1, rank2, bias=False)
        self.logix_lora_C = nn.Linear(rank2, out_features, bias=False)
        ...
        self.initialize_jl_matrix(rank1, in_features, self.logix_lora_A.weight, lora_layer_type='A')
        self.initialize_jl_matrix(rank2, out_features, self.logix_lora_C.weight, lora_layer_type='C')
    ...
    def initialize_jl_matrix(self, k, d, weight, lora_layer_type):

        if lora_layer_type == 'A':
            jl_matrix = torch.randn(k, d)
        elif lora_layer_type == 'C':
            jl_matrix = torch.randn(d, k)
        else:
            raise ValueError('Invalid lora_layer_type')
        
        # Scale the matrix to preserve vector norms
        scaling_factor = math.sqrt(d / k)
        jl_matrix *= scaling_factor
        
        # Assign the JL matrix to the weight
        with torch.no_grad():
            weight.copy_(jl_matrix)
