...
# Import InRunShapley engine if available
try:
    import sys
    import os
...
class GhostEngineManager:
    ...
    def _initialize_engine(self):
        ...
        elif self.config.method == 'InRunShapley':
            self._initialize_inrun_shapley_engine()
    ...
    def _initialize_inrun_shapley_engine(self):
        """Initialize InRunShapleyEngine with all required setup."""
        if not INRUN_SHAPLEY_AVAILABLE:
            raise ImportError("InRunShapleyEngine not available. Make sure the module is properly imported.")
        
        print("[INFO] Initializing InRunShapleyEngine ...")
        
        # Prepare directory for saving dot products and Shapley values
        dot_prod_save_path = os.path.join(self.config.result_dir, "grad_dotprods")
        if self.ddp_info['master_process']:
            os.makedirs(dot_prod_save_path, exist_ok=True)
        
        # Get InRunShapley specific parameters
        ...
        # Initialize the engine
        self.engine = InRunShapleyEngine(
            module=self.model,
            val_batch_size=self.config.val_batch_size,
            loss_reduction='mean',
            use_dummy_bias=True,
        ...
        # Validate that validation data is provided
        if self.X_val is None or self.Y_val is None:
            raise ValueError("X_val and Y_val are required for InRunShapley method")
        
        # Attach validation data to the engine
        self.engine.attach_and_store_valset(self.X_val, self.Y_val)
        
        print("[INFO] InRunShapleyEngine initialized successfully.")
    ...
    def should_save_metrics(self, iter_num: int) -> bool:
        ...
        elif self.config.method == 'InRunShapley' and iter_num > 0:
            return iter_num % self.config.dot_prod_save_interval == 0
    ...
    def save_metrics(self, iter_num: int):
        ...
        elif self.config.method == 'InRunShapley' and self.engine:
            # Save dot products
            if hasattr(self.engine, 'save_dot_product_log'):
                self.engine.save_dot_product_log(iter_num=iter_num)
            # Shapley values are saved separately via should_save_shapley
    ...
    def prepare_forward_input(self, X_train, Y_train):
        ...
        """
        if self.config.method in ['GradDotProd', 'InRunShapley'] and self.X_val is not None:
    ...
    def cleanup(self):
        ...
        if (self.config.method in ['GradDotProd', 'InRunShapley'] and 
        ...
        # Save final Shapley values for InRunShapley
        if (self.config.method == 'InRunShapley' and 
            hasattr(self.engine, 'save_shapley_values')):
            try:
                self.engine.save_shapley_values(iter_num=-1)
                self.engine.save_shapley_history(iter_num=-1)
            except Exception as e:
                print(f"Error saving Shapley values during cleanup: {e}")
        elif self.config.method == 'GradProjLora':
