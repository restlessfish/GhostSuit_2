...
class BaseValidator:
    ...
    def should_validate(self, step: int) -> bool:
        # Always run validation on the final training step even if it doesn't align with the freq
        return (
            step == 1
            or step % self.job_config.validation.freq == 0
            or step == self.job_config.training.steps
        )
