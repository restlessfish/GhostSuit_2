...
class HFTransformerModelArgs(PretrainedConfig, BaseModelArgs):
    ...
    def update_from_config(self, job_config: JobConfig):
        ...
        # HF-mapped fields (dim, n_layers, etc.) should stay from the HF config;
        # only apply flavor overrides for non-mapped fields or when explicitly set.
        mapped_keys = set(self._tt_to_hf_attribute_map.keys())
        ...
        if not hasattr(self, "inter_dim"):  # Only recompute for llama-style configs
